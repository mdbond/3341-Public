...
Ref sort(Ref list) {
    ...
    Ref rightSubList = subList(list, pivot, 1);
    mutable int anotherPivot = -1;
    if (isNil(rightSubList) == 0)
        anotherPivot = (int)left(rightSubList);
    Ref lists = [ [ sort(subList(list, pivot, -1)) . sort(subList(rightSubList, anotherPivot, -1)) ] .
                  sort(subList(rightSubList, anotherPivot, 1)) ];
    return concat((Ref)left((Ref)left(lists)),
                  concat(subList(list, pivot, 0),
                         concat((Ref)right((Ref)left(lists)),
                                concat(subList(rightSubList, anotherPivot, 0),
                                       (Ref)right(lists)))));
    /*Ref lists = [ sort(subList(list, pivot, -1)) . sort(subList(list, pivot, 1)) ];
    return concat((Ref)left(lists), (Ref)right(lists));*/
}
